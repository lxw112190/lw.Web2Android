...
GUI 参考 `lw.Web2App` 的设计语言，支持 Per-Monitor V2 高 DPI、后台工作线程和 15 步构建状态。v0.2.7 将网页来源与应用设置改为固定尺寸的左右双栏布局；左侧项目支持二维码直接编译进 EXE。GUI 只把表单映射为 `ProjectConfig`，所有 APK 逻辑仍由 `BuildPipeline` 完成。
...
GitHub Actions 使用统一的 `lw.Web2Android CI` workflow。CI 会从已安装 SDK 组装扁平最小工具链，检出固定版本的 `wechat-article-formatter`，构建真实 React/Vite `dist` 并生成 Demo APK；Remote、Unicode Assets、固定宽度网页与自定义图标模式作为独立集成测试。CI 会拒绝反斜杠、重复条目、意外打入的 `project.json` 或缺失的中文文件。CLI、GUI、Runtime、初始化脚本、MIT License、Demo APK、发行元数据和总校验文件会合并到单个 `lw-Web2Android-v0.2.7-windows-x64` Artifact。公开发行包不重新分发 Android SDK。
