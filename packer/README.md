...
GitHub Actions 使用统一的 `lw.Web2Android CI` workflow。CI 会从已安装 SDK 组装扁平最小工具链，检出固定版本的 `wechat-article-formatter`，构建真实 React/Vite `dist` 并生成 Demo APK；Remote、Unicode Assets 与固定宽度网页模式作为独立集成测试。CI 会拒绝反斜杠、重复条目、意外打入的 `project.json` 或缺失的中文文件。CLI、GUI、Runtime、初始化脚本、MIT License、Demo APK、发行元数据和总校验文件会合并到单个 `lw-Web2Android-v0.2.5-windows-x64` Artifact。公开发行包不重新分发 Android SDK。
