...
GUI 参考 `lw.Web2App` 的设计语言，支持 Per-Monitor V2 高 DPI、后台工作线程和 15 步构建状态。v0.2.7 将网页来源与应用设置改为固定尺寸的左右双栏布局；v0.2.8 在本地模式增加受限的外部文本系统集成；v0.2.9 为 Windows EXE 和标题栏增加统一产品图标，并在二维码下方增加项目主页与最新版本入口；v0.2.11 增加 Runtime Schema 与 DEX 完整性校验。左侧项目支持二维码直接编译进 EXE。GUI 只把表单映射为 `ProjectConfig`，所有 APK 逻辑仍由 `BuildPipeline` 完成。
