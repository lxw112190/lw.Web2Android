...
GUI 参考 `lw.Web2App` 的设计语言，支持 Per-Monitor V2 高 DPI、后台工作线程和 15 步构建状态。v0.2.7 将网页来源与应用设置改为固定尺寸的左右双栏布局；v0.2.8 在本地模式增加受限的外部文本系统集成；v0.2.9 为 Windows EXE 和标题栏增加统一产品图标，并在二维码下方增加项目主页与最新版本入口；v0.2.11 增加 Runtime Schema 与 DEX 完整性校验；v0.2.12 完整支持 HTML 图片 `capture` 调用系统相机；v0.2.13 改进工具链初始化的长路径兼容、阶段诊断和失败现场保留。左侧项目支持二维码直接编译进 EXE。GUI 只把表单映射为 `ProjectConfig`，所有 APK 逻辑仍由 `BuildPipeline` 完成。
...
本地项目可增加可选的 `externalContent` 对象，启用接收分享文本或注册文本/配置文件打开方式：
...
可用预设为 `text-basic`、`text-config` 和 `code-config`；也可用 `extensions`、`fileNames`、`mimeTypes` 字符串数组追加允许项。`acceptOctetStream` 默认关闭。Remote URL 模式禁止启用此功能。
